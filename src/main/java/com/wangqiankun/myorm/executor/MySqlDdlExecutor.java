package com.wangqiankun.myorm.executor;

import com.wangqiankun.myorm.entity.TableInfo;
import com.wangqiankun.myorm.entity.View;
import com.wangqiankun.myorm.utils.SqlBuilder;
import lombok.extern.slf4j.Slf4j;
import org.springframework.stereotype.Component;

import javax.annotation.Resource;
import javax.sql.DataSource;
import java.sql.Connection;
import java.sql.PreparedStatement;
import java.sql.SQLException;
import java.util.Objects;

/**
 * 功能描述:
 *
 * @author 王乾坤
 * @version 1.0
 * @date 2022/6/29 17:17
 */
@Slf4j
@Component
public class MySqlDdlExecutor implements DdlExecutor {
    @Resource
    private DataSource mySqlDataSource;

    @Override
    public boolean createTable(TableInfo t) {
        Connection connection;
        PreparedStatement createTableStatement = null;
        PreparedStatement dropTableStatement = null;
        try {
            connection = mySqlDataSource.getConnection();
            if (t.isDropIfPresent()) {
                dropTableStatement = connection.prepareStatement("drop table if exists " + t.getTableName());
                dropTableStatement.execute();
            }
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }

        String sql = SqlBuilder.buildCreateTableSql(t);
        sql += "charset=" + t.getCharSet();
        try {
            log.info("执行SQL语句：\n" + sql);
            createTableStatement = connection.prepareStatement(sql);
            createTableStatement.execute();
            return true;
        } catch (SQLException e) {
            e.printStackTrace();
            log.info("创建表失败" + System.lineSeparator() + sql);
        } finally {
            try {
                if (Objects.nonNull(connection)) {
                    connection.close();
                }
                if (Objects.nonNull(createTableStatement)) {
                    createTableStatement.close();
                }
                if (Objects.nonNull(dropTableStatement)) {
                    dropTableStatement.close();
                }
            } catch (SQLException e) {
                e.printStackTrace();
            }
        }
        return false;
    }

    @Override
    public boolean createView(View view) {
        //TODO
        return false;
    }


}